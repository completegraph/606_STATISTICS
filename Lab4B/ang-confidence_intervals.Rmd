...
Ensure the Lab is repeatable by setting the seed at startup.
```{r}
set.seed( 1483)
...
The sample statistics show the distribution is skewed right.  The mean is 1518.  The median 1503 is lower.
This is because house sizes have to be a minimum size to be habitable, but there is no size upper limit.
The code chunk is below.

The typical point estimate size of a house is the mean (1518).
The typical size of a house is between 1010.71 to 2024.79 square feet using the 1 standard deviation interval around the sample mean to define criteria to be "typical".
...
hist(samp)
( s1 = sd(samp) )
(limits = c( mean(samp) - s1, mean(samp) + s1) )
...
No.  Another student's distribution would not likely be identical to mine because each of us takes a random sample.
Yes.  We would expect our two samples to have similar statistics since they are drawn from an identical distribution.
...
**(Alexander Ng's)**  
There are 3 conditions required for the confidence interval to be valid:  
a) $n >= 30$ the sample size must be sufficiently large  
b) the distribution's skew must not be too large.  
...
**(Alexander Ng's RESPONSE)**  
95% confidence means that if we take repeated trials of drawing samples and building confidence intervals that, in 95 percent of the trials, the true population mean is contained in the confidence interval.
...
**(Alexander Ng's Response)**  
The true population mean of house square footage lies the 95% confidence interval calculated above 1389.451 and 1646.049.
Since I am not working in a lab, we cannot compare to other students' work.
...
We expect the proportion of confidence intervals to capture the true mean to be 95%.   
Even though we cannot gather other students' responses, simulation can give the required answers.
...
We choose a 90% confidence interval.  This gives a critical value of $$Z= 1.644854$$
which is computed by using the qnorm function:

```{r}
(z90 = -qnorm(0.05) )

lower_vector90 <- samp_mean - z90 * samp_sd / sqrt(n) 
upper_vector90 <- samp_mean + z90 * samp_sd / sqrt(n)

plot_ci( lower_vector90, upper_vector90, mean(population))

```

We observe 7 red lines where the population mean falls outside of the confidence interval.  This is slightly higher than the expected 5 red lines of 50 trials.   The deviation in the confidence interval count is due to the sampling variation of the 50 trials.
